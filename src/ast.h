...
/*
    Notes here:
        ...
        * (寄, Solution 见19) 15. How to Handle `if-else`
        ...
        25. 假设全局变量，赋的值一定也可计算? (是的，不然就是语义错误)
        ...
        26. 原文档给的文法有一点逆天，其实是有部分歧义的：
          ...
          26.2 : 归结 调用函数 和 一个变量的时候，会产生  规约 & 推导 冲突，这是由于两个东西都以 IDENT 开头，且前者还需要一个 '('
          26.2 Solution : 不用管，因为我们的文法可以向后 check 一个 char，就可以完全避免这个冲突
              - 调用函数的 IDENT 后严格以 '(' 开头
